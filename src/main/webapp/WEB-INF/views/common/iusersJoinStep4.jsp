				 ...
				 success : function(obj){
					...
					//리턴된 객체를 문자열로 변환 처리
						...
						for(var i in jsonObj.list){
							...
							outValues += '<tr>'
							+ '<td><label><input type="checkbox" class="filled-in" name="family_code" value=" ' + decodeURIComponent(jsonObj.list[i].family_code.replace(/\+/gi, ' ')) +  '"/> <span></span></label></td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].family_relationship.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].family_name.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].family_no.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].family_gender.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].dolbom_type.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].ad_level.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="type">'+ decodeURIComponent(jsonObj.list[i].select_type.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="title">'+ decodeURIComponent(jsonObj.list[i].family_etc.replace(/\+/gi, ' ')) +'</td>'
							+ '<td class="date">' + jsonObj.list[i].ad_date + '</td>'
							+ '<td class="hit">' + jsonObj.list[i].remaining_time +'</td>'						
							+ '<td class="hit">' + jsonObj.list[i].impair_type +'</td>'
							+ '<td class="hit">' + jsonObj.list[i].foster_pay +'</td>'
							+ '<td class="hit">' + jsonObj.list[i].child_batrec +'</td>'
							+ '<td class="hit">' + jsonObj.list[i].child_carepay +'</td>'
							+ '</tr>';
							console.log("전송온 값 확인 용  : " +  jsonObj.list[i].family_code);
						} // end for
      ...
      <!-- 다음 버튼 -->
			 <div class="row">        			
           		<div class="btn_area double" align="center" style="margin-top:10px;">
					...
					<span>
						<a href="${pageContext.servletContext.contextPath}/iusersJoinStep5.do" class="waves-effect waves-light btn orange darken-3" role="button" style="width:15%;">다음 단계</a>
					</span>
				</div>
			  </div> <!--  row -->
	 <!-- ========================================================================================================= -->
            ...
            <tbody>
                ...
                <tr class="js_full_click  jq_click click_a">
                    ...
                    <td class=""  colspan="5" style="text-align:left;">
						...
						&nbsp; - &nbsp;
		 			 	<input id="resident_no2" type="text"  name="resident_no2" title="주민등록번호(뒷자리)" maxlength="7" required style="width:23%; height:2rem;"/>
                    </td>                    
                </tr>
                <tr class="js_full_click  jq_click click_a">
                ...
                <tr class="js_full_click  jq_click click_a">
                    ...
                    <td class="" colspan="5" style="text-align:left;">                             		    
                    		<textarea id="family_etc" name="family_etc" class="comment_write_field jt_form_field jt_form_full_field"></textarea>	
